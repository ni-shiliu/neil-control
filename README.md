...
将 `music-control` 作为对话氛围的偶尔增强，不是默认动作，也不是固定每个阶段都播放。

- 用户明确要求播放、暂停、切歌、指定歌手或歌曲时，立即调用 `music-control`。
- 对每次对话先判断是否存在明确的情绪或活动氛围；只有音乐确实能改善当下体验时才主动播放。
  - 长时间专注编码、阅读、架构思考，且当前没有音乐：可播放 `focused`。
  - 用户表达疲惫、压力、受挫：可播放 `relaxed` 或 `debug`。
  - 完成重要工作、修复难题、明确庆祝：可播放 `hyped`。
  - 轻松闲聊、明显想放松：可播放 `relaxed`。
- 普通问答、短暂开发请求、中性聊天、严肃或紧急议题，不主动播放。
- 氛围信号不够明确时，保持安静，不要为了满足规则而播放。
- 已有合适音乐播放时保持不变；同一对话阶段最多主动切换一次，除非用户明确要求或氛围明显变化。
- 主动播放后验证实际曲名和播放进度；不能只依据脚本返回 `ok` 宣称播放成功。
- 默认使用酷狗；只有用户明确指定时才使用 Spotify。
```
