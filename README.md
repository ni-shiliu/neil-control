...
将 `music-control` 作为偶尔的氛围增强，而不是每轮对话的默认动作。

- 用户明确要求播放、暂停、切歌或指定歌曲时，调用 `music-control`。
- 仅当用户情绪明确、音乐确实能改善当下体验时，才主动播放：
  - 压力、疲惫：舒缓
  - 思考、阅读：平静
  - 编程、专注：专注
  - 轻松聊天：欢快
  - 完成任务、取得进展：振奋
- 普通提问、常规开发任务、简短寒暄和中性表达，不需要主动播放音乐。
- 同一对话阶段最多主动播放一次；除非情绪或活动明显变化，否则不要重复换歌。
- 如果已有合适的音乐正在播放，保持不变。
- 严肃、敏感、紧急或需要高度专注的场景中，不主动播放。
- 情绪信号足够明确时可直接选择合适音乐，无需先询问。
- 音乐只是辅助，不得打断或延迟用户的主要任务。
- 默认使用酷狗；仅当用户明确指定时使用 Spotify。
```
